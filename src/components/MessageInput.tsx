...
export function MessageInput({ conversationId, lang }: MessageInputProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4 border-t border-gray-200 bg-white sticky bottom-0">
      ...
      {mutation.isError && <p className="text-sm text-red-600 mb-2">{t.messages.loadError}</p>}
      <div className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={mutation.isPending}
          className="px-6 py-2 bg-lbc-orange text-white rounded-full hover:bg-opacity-90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {t.messages.send}
        </button>
      </div>
